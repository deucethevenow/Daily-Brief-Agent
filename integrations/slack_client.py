...
class SlackClient:
    ...
    def __init__(self):
        ...
        self.team_channel_id = getattr(Config, 'SLACK_TEAM_CHANNEL_ID', None) or None
        if not self.team_channel_id:
            logger.warning("SLACK_TEAM_CHANNEL_ID not set — team mention summaries will be skipped")
    ...
    def send_team_mention_summary(self, user_tasks: List[Dict[str, Any]], date_str: str) -> Dict[str, Any]:
        """Send unanswered mentions to #team: clean header + one combined thread reply.

        Posts a header message to the channel, then a single thread reply
        listing all people so the main feed stays clean.

        Args:
            user_tasks: List of dicts with keys:
                - user_name: Asana display name
                - slack_user_id: Slack user ID for @mention (or None)
                - task_gid: Asana parent task GID
                - mention_count: Number of unanswered comments
            date_str: Formatted date string (e.g., "Mar 28")

        Returns:
            Slack API response for the parent message
        """
        if not self.team_channel_id:
            logger.info("No team channel configured — skipping team mention summary")
            return None

        if not user_tasks:
            logger.info("No team mentions to summarize")
            return None

        # --- Parent message: clean header only ---
        total_mentions = sum(e['mention_count'] for e in user_tasks)
        header_blocks = [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": f"📬 Unanswered @Mentions — {date_str}"
                }
            },
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": f"*{len(user_tasks)}* team members have *{total_mentions}* Asana comments that need responses.\nSee thread for details :thread:"
                }
            },
            {
                "type": "context",
                "elements": [{
                    "type": "mrkdwn",
                    "text": "Auto-generated by Daily Brief Agent · Respond in Asana to clear these"
                }]
            }
        ]

        text = f"📬 {len(user_tasks)} team members have unanswered @mentions — {date_str}"
        ...
        try:
            ...
            logger.info(f"Team mention header sent to {self.team_channel_id}")

            # --- Single thread reply with all people combined ---
            lines = []
            for entry in user_tasks:
                name = entry['user_name']
                slack_id = entry.get('slack_user_id')
                task_gid = entry['task_gid']
                count = entry['mention_count']
                task_url = f"https://app.asana.com/0/0/{task_gid}"

                user_ref = f"<@{slack_id}>" if slack_id else f"*{name}*"
                comment_word = "comment" if count == 1 else "comments"

                lines.append(f"{user_ref} — {count} {comment_word} need{'s' if count == 1 else ''} a response\n:point_right: <{task_url}|View & respond in Asana>")
            ...
            try:
                ...
                logger.info(f"Team mention thread reply sent ({len(user_tasks)} users)")
            except SlackApiError as e:
                logger.error(f"Failed to post thread reply: {e.response['error']}")

            return response
        except SlackApiError as e:
            logger.error(f"Error sending team mention summary: {e.response['error']}")
            raise
