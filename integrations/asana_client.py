...
class AsanaClient:
    ...
    def create_respond_to_mentions_task(self, mentions: List[Dict[str, Any]],
        ...
        # Group mentions by person they're for
        by_user = {}
        ...
        # Build the task description with all mentions
        notes_parts = [
        ...
        # Add mentions grouped by user
        for user_name, user_mentions in by_user.items():
            notes_parts.append("")
            notes_parts.append(f"━━━ FOR {user_name.upper()} ({len(user_mentions)} mentions) ━━━")
            notes_parts.append("")
            ...
            for i, mention in enumerate(user_mentions, 1):
                ...
                notes_parts.extend([
                    f"─── Mention #{i} ───",
                    f"📋 Task: {mention.get('task_name', 'Unknown Task')}",
                    f"📁 Project: {mention.get('project_name', 'No Project')}",
                    f"🔗 Link: {mention.get('task_url', 'No link')}",
                    f"👤 From: {mention.get('author_name', 'Unknown')} ({time_str})",
                    "",
                    f"💬 Comment:",
        ...
        # Determine assignee - if multiple users, assign to first one or config YOUR_NAME
        if assignee_name:
            assignee = assignee_name
        elif len(by_user) == 1:
            assignee = list(by_user.keys())[0]
        else:
        ...
        try:
            task_data = {
                ...
                'notes': task_notes,
                'workspace': self.workspace_gid,
                'due_on': today.strftime('%Y-%m-%d'),  # Due today
            }
            ...
            logger.info(f"Created respond-to-mentions task: {task_gid} with {len(mentions)} mentions")

            # Note: Subtasks removed - the main task description contains all mention details
            # with clickable links. This is cleaner than having subtasks without proper links.
